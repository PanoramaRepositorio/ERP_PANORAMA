...
{
    public class ReporteProductoCatalogoNovioRegaloDL
    {
        public List<ReporteProductoCatalogoNovioRegaloBE> Listado(int IdNovioRegalo)
        {
            Database db = DatabaseFactory.CreateDatabase("cnErpPanoramaBD");
            DbCommand dbCommand = db.GetStoredProcCommand("usp_rptProductoCatologoNovioRegalo");
            db.AddInParameter(dbCommand, "pIdNovioRegalo", DbType.Int32, IdNovioRegalo);

            IDataReader reader = db.ExecuteReader(dbCommand);
            List<ReporteProductoCatalogoNovioRegaloBE> ProductoCatalogoPedidolist = new List<ReporteProductoCatalogoNovioRegaloBE>();
            ReporteProductoCatalogoNovioRegaloBE ProductoCatalogoPedido;
            while (reader.Read())
            {
                ProductoCatalogoPedido = new ReporteProductoCatalogoNovioRegaloBE();
