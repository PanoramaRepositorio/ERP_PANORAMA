...
{
    public class ReportePedidoRutaAnioDL
    {
        public List<ReportePedidoRutaAnioBE> Listado(DateTime FechaDesde, DateTime FechaHasta)
        {
            Database db = DatabaseFactory.CreateDatabase("cnErpPanoramaBD");
            DbCommand dbCommand = db.GetStoredProcCommand("usp_rptPedidoRutaAnio");
            db.AddInParameter(dbCommand, "@pFechaDesde", DbType.DateTime, FechaDesde);
            db.AddInParameter(dbCommand, "@pFechaHasta", DbType.DateTime, FechaHasta);

            IDataReader reader = db.ExecuteReader(dbCommand);
            List<ReportePedidoRutaAnioBE> Pedidolist = new List<ReportePedidoRutaAnioBE>();
            ReportePedidoRutaAnioBE Pedido;
            while (reader.Read())
            {
                Pedido = new ReportePedidoRutaAnioBE();
                Pedido.Periodo = Int32.Parse(reader["Periodo"].ToString());
                Pedido.Anio = reader["Anio"].ToString();
