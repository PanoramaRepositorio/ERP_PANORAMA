...
{
    public class ReporteProductoCatalogoDL
    {
        public List<ReporteProductoCatalogoBE> Listado(int IdEmpresa)
        {
            Database db = DatabaseFactory.CreateDatabase("cnErpPanoramaBD");
            DbCommand dbCommand = db.GetStoredProcCommand("usp_rptProductoCatologo");
            db.AddInParameter(dbCommand, "pIdEmpresa", DbType.Int32, IdEmpresa);
            
            IDataReader reader = db.ExecuteReader(dbCommand);
            List<ReporteProductoCatalogoBE> Productolist = new List<ReporteProductoCatalogoBE>();
            ReporteProductoCatalogoBE Producto;
            while (reader.Read())
            {
                Producto = new ReporteProductoCatalogoBE();
                ...
                Producto.StockBultos = Int32.Parse(reader["StockBultos"].ToString());
