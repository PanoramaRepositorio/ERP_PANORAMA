...
{
    public partial class frmRegFacturacionEmpresa : DevExpress.XtraEditors.XtraForm
    {
        #region "Propiedades"
        ...
        public frmRegFacturacionEmpresa()
        {
            InitializeComponent();
        }

        private void frmRegFacturacionEmpresa_Load(object sender, EventArgs e)
        {
        ...
        private void tlbMenu_NewClick()
        {
            try
            {
                frmRegFacturacionEmpresaEdit objManDocumentoVenta = new frmRegFacturacionEmpresaEdit();
                objManDocumentoVenta.pOperacion = frmRegFacturacionEmpresaEdit.Operacion.Nuevo;
                ...
                if (TipoDoc == Parametros.intTipoDocBoletaElectronica || TipoDoc == Parametros.intTipoDocFacturaElectronica || TipoDoc == Parametros.intTipoDocNotaCreditoElectronica)
                {
                    ...
                    if (XtraMessageBox.Show("Está seguro de dar de BAJA el Comprobante electrónico?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        if (!ValidarIngreso())
                        {
                            ...
                            XtraMessageBox.Show("El documento se dió de baja correctamente", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Cargar();
                        }
                    }
                    #endregion
            ...
            try
            {
                if (mLista.Count > 0)
                {
                    ...
                    if (lstReporte.Count > 0)
                    {
                        ...
                        objRptDocumento.VerRptDocumentoVentaEmpresaTraslado(lstReporte, objE_DocumentoVenta.IdTipoDocumento);
        ...
        private void imprimirToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (mLista.Count > 0)
                {
                    ...
                    {
                        List<ReporteDocumentoVentaBE> lstReporte = null;
                        lstReporte = new ReporteDocumentoVentaBL().ListaCliente(Convert.ToInt32(objE_DocumentoVenta.IdEmpresa), Convert.ToInt32(objE_DocumentoVenta.IdCliente), objE_DocumentoVenta.Fecha,objE_DocumentoVenta.Fecha);// deDesde.DateTime, deHasta.DateTime);
        ...
        private void Cargar()
        {
            mLista = new DocumentoVentaBL().ListaEmpresaTraslado(0, deDesde.DateTime, deHasta.DateTime);
