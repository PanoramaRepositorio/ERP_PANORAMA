...
{
    public partial class frmRegFacturacionEmpresaEdit : DevExpress.XtraEditors.XtraForm
    {
        #region "Propiedades"
        ...
        public frmRegFacturacionEmpresaEdit()
        {
            InitializeComponent();
        }

        private void frmRegFacturacionEmpresaEdit_Load(object sender, EventArgs e)
        {
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            if (!ValidarIngreso())
            {
                ...
                {
                    if (mListaDocumentoVentaDetalleOrigen.Count <= 15)
                    {
                    ...
                    {
                        InsertarDocumentoVentaVarios(15);
                        if (XtraMessageBox.Show("Esta seguro de imprimir la factura de venta?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            #region "Impresion"
                            if (Convert.ToInt32(cboEmpresa.EditValue) == Parametros.intPanoraramaDistribuidores)//Verificando Panorama
                ...
                if (Convert.ToInt32(cboDocumento.EditValue) == Parametros.intTipoDocBoletaElectronica || Convert.ToInt32(cboDocumento.EditValue) == Parametros.intTipoDocFacturaElectronica) //Documentos Electrónicos
                {
                    if (mListaDocumentoVentaDetalleOrigen.Count <= 15)
                    {
                        InsertarDocumentoVenta();//Revisar
                ...
                if (gvDocumentoDetalle.RowCount > 0)
                {
                    if (Convert.ToInt32(cboMoneda.EditValue) == Parametros.intSoles) //soles
                    {
                        ...
                        foreach (var item in mListaDocumentoVentaDetalleOrigen)
                        {
                            ...
                            decPorcentajeDescuento = (1 - Convert.ToDecimal(txtTipoCambio.EditValue))*100;

                            gvDocumentoDetalle.SetRowCellValue(posicion, gvDocumentoDetalle.Columns["PrecioVenta"], decPrecioVenta);
                            gvDocumentoDetalle.SetRowCellValue(posicion, gvDocumentoDetalle.Columns["PorcentajeDescuento"], decPorcentajeDescuento);
        ...
        private void InsertarDocumentoVenta()
        {
            if (!ValidarIngreso())
            {
                ...
                //mListaNumero = new NumeracionDocumentoBL().ObtenerCorrelativoPeriodo(Convert.ToInt32(cboEmpresa.EditValue), Convert.ToInt32(cboDocumento.EditValue), Parametros.intPeriodo);//Convert.ToInt32(deFecha.DateTime.Year));
                mListaNumero = new NumeracionDocumentoBL().ObtenerCorrelativoSerie(Convert.ToInt32(cboEmpresa.EditValue), Convert.ToInt32(cboDocumento.EditValue), cboSerie.Text);//Convert.ToInt32(deFecha.DateTime.Year));
                if (mListaNumero.Count > 0)
                {
                    Numero = FuncionBase.AgregarCaracter((mListaNumero[0].Numero + 1).ToString(), "0", 8);
        ...
        private bool ValidarIngreso()
        {
            ...
            if (Convert.ToInt32(cboSerie.EditValue) == 0  && cboSerie.Text.Trim().ToString() == "")
            {
                strMensaje = strMensaje + "- Seleccionar Serie del documento.\n";
                flag = true;
            }
        ...
        private DataTable CargarTipoDocumento()
        {
            ...
            dr = dt.NewRow();
            dr["Id"] = 12;
            dr["Descripcion"] = "BOLETA ELECTRONICA";
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Id"] = 13;
            dr["Descripcion"] = "FACTURA ELECTRONICA";
            dt.Rows.Add(dr);

            //dr = dt.NewRow();
            //dr["Id"] = 96;
            //dr["Descripcion"] = "BOLETA VENTA EMPRESA (TRASLADO)";
            //dt.Rows.Add(dr);
            //dr = dt.NewRow();
            //dr["Id"] = 97;
            //dr["Descripcion"] = "FACTURA VENTA EMPRESA (TRASLADO)";
            //dt.Rows.Add(dr);
            return dt;
        }
        ...
        private void cboDocumento_EditValueChanged(object sender, EventArgs e)
        {
            if (pOperacion == Operacion.Nuevo)
            {
                ...
                if (lst_Numeracion.Count > 0)
                {
                    ...
                    cboSerie.Properties.ReadOnly = false;
                    if (Convert.ToInt32(cboDocumento.EditValue) == Parametros.intTipoDocBoletaElectronica)
                        cboSerie.Text = "B009";
                    else if (Convert.ToInt32(cboDocumento.EditValue) == Parametros.intTipoDocFacturaElectronica)
                        cboSerie.Text = "F009";
                    cboSerie.Properties.ReadOnly = true;
                }
                else
                {
