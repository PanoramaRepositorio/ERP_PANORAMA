...
{
    public partial class frmReprogramarVisita : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmReprogramarVisita()
        {
            InitializeComponent();
        }

        private void frmReprogramarVisita_Load(object sender, EventArgs e)
        {
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objAgenda.IdSituacion = Parametros.intReprogramado;
                objAgenda.FechaAgenda = Convert.ToDateTime(denavAgenda.EditValue);

                //objBL_Agenda.ReprogramaVisita(objAgenda);
                //XtraMessageBox.Show("Se reprograma satisfactoriamente la cita.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                //this.Close();
                ...
                string mensaje_exitoso = "Se reprogramó satisfactoriamente la cita.";
                ...
                else if (agendaXvendedora.Count > 0)
                {
                    ...
                    {
                        DialogResult dr = XtraMessageBox.Show(fecha + "¿Desea reprogramar una visita en el mismo dia?", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                    ...
                    else
                    {
                        ...
                        if (frmAutoriza.Edita && frmAutoriza.FlagMaster)
                        {
                            ...
                            {
                                XtraMessageBox.Show(fecha + "No puede reprogramar una visita en ese día.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                        }
                        else
                        {
                            XtraMessageBox.Show(fecha + "Usted no está autorizado para reprogramar una visita en ese día.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                    }
                }
                this.Close();
