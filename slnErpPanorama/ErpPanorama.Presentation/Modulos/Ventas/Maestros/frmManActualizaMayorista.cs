...
{
    public partial class frmManActualizaMayorista : DevExpress.XtraEditors.XtraForm
    {
        #region "Propiedades"
        ...
        public frmManActualizaMayorista()
        {
            InitializeComponent();
        }

        private void frmManActualizaMayorista_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!ValidarIngreso())
                {
                    if (XtraMessageBox.Show("Esta seguro(a) de cambiar al cliente a Mayorista?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        objBL_Cliente.ActualizaMayorista(objE_Cliente);
                    }
        ...
        private bool ValidarIngreso()
        {
            ...
            if(Parametros.intPerfilId != Parametros.intPerAdministrador && Parametros.intPerfilId != Parametros.intPerJefeCanalMayorista)
            {
                strMensaje = strMensaje + "- Ud. no está autorizado para realizar esta operación. Sólo los usuario de tipo administrador.\n";
                flag = true;
            }
