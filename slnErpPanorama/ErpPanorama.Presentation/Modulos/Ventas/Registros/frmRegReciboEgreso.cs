...
{
    public partial class frmRegReciboEgreso : DevExpress.XtraEditors.XtraForm
    {
        #region "Propiedades"
        ...
        public frmRegReciboEgreso()
        {
            InitializeComponent();
        }

        private void frmRegReciboEgreso_Load(object sender, EventArgs e)
        {
            ...
            if (Parametros.strUsuarioLogin == "master" || Parametros.strUsuarioLogin == "rcastañeda" || Parametros.intPerfilId == Parametros.intPerAdministrador)
            {
            ...
            try
            {
                ...
                if (XtraMessageBox.Show("Esta seguro de eliminar el registro, Esto podría afectar el estado de cuenta?", this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (!ValidarIngreso())
                    {
                        ...
                        objE_Pagos.IdPago = int.Parse(gvReciboEgreso.GetFocusedRowCellValue("IdPago").ToString());
                        ...
                        objE_MovimientoCaja = new MovimientoCajaBL().SeleccionaNumero(int.Parse(gvReciboEgreso.GetFocusedRowCellValue("IdEmpresa").ToString()), Parametros.intTipoDocReciboEgreso, gvReciboEgreso.GetFocusedRowCellValue("NumeroDocumento").ToString());
        ...
        {
            if (gvReciboEgreso.RowCount > 0)
            {
                int IdPagoDocumento = int.Parse(gvReciboEgreso.GetFocusedRowCellValue("IdPago").ToString());
        ...
        private void tlbMenu_ExportClick()
        {
            ...
            string _fileName = "ListadoReciboEgreso";
            FolderBrowserDialog f = new FolderBrowserDialog();
            f.ShowDialog(this);
            if (f.SelectedPath != "")
            {
                Cursor = Cursors.AppStarting;
                gvReciboEgreso.ExportToXls(f.SelectedPath + @"\" + _fileName + ".xls");
        ...
        private void gvReciboEgreso_DoubleClick(object sender, EventArgs e)
        {
        ...
        private void Cargar()
        {
            dt = FuncionBase.ToDataTable(new PagosBL().ListaTodosActivo(Parametros.intEmpresaId, Convert.ToInt32(cboCaja.EditValue), Convert.ToDateTime(deFecha.EditValue),Convert.ToDateTime(deFecha.EditValue), Parametros.intTipoDocReciboEgreso));
            gcReciboEgreso.DataSource = dt;
        }
        ...
        public void InicializarModificar()
        {
            if (gvReciboEgreso.RowCount > 0)
            {
                ...
                lstMovimientoCaja = new MovimientoCajaBL().ListaPagos(int.Parse(gvReciboEgreso.GetFocusedRowCellValue("IdPago").ToString()));
                ...
                PagosBE objReciboEgreso = new PagosBE();
                objReciboEgreso.IdPago = int.Parse(gvReciboEgreso.GetFocusedRowCellValue("IdPago").ToString());
                objReciboEgreso.Fecha = DateTime.Parse(gvReciboEgreso.GetFocusedRowCellValue("Fecha").ToString());
                ...
                objRegReciboEgresoEdit.IdPago = objReciboEgreso.IdPago;
                objRegReciboEgresoEdit.FechaD = objReciboEgreso.Fecha;
                objRegReciboEgresoEdit.StartPosition = FormStartPosition.CenterParent;
                objRegReciboEgresoEdit.ShowDialog();
        ...
        private bool ValidarIngreso()
        {
            ...
            if (gvReciboEgreso.GetFocusedRowCellValue("IdPago").ToString() == "")
            {
        ...
        private void HabilitarBotones()
        {
            ...
            gcReciboEgreso.Enabled = true;
        }

        private void DesHabilitarBotones()
        {
            tlbMenu.Enabled = false;
            gcReciboEgreso.Enabled = false;
        }
        ...
        private void Imprimir(int IdPagoI)
        {
            ...
            if (objE_Pago != null)
            {
                ...
                //objTalon = new TalonBL().SeleccionaCajaDocumento(Parametros.intEmpresaId, Parametros.intTiendaId, Parametros.intCajaId, Parametros.intTipoDocReciboEgreso);
