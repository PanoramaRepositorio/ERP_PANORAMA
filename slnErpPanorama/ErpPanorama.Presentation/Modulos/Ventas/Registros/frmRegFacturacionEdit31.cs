    ...
    public partial class frmRegFacturacionEdit : DevExpress.XtraEditors.XtraForm
    {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (objE_Pedido != null)
                    {
                        ...
                        if (Parametros.intTiendaId == Parametros.intTiendaUcayali)
                        {
                            ...
                            {
                                if (!objE_MovimientoPedido.Chequeado && objE_Pedido.IdCliente!= 236149)  // DECORATEX para sin los procesos de chequeo, embalaje
                                {
                                    if (objE_Pedido.Despachar == "ALMACEN")
                                    {
        ...
        private void InsertarDocumentoVenta()
        {
            if (!ValidarIngreso())
            {
                ...
                if (pOperacion == Operacion.Nuevo)
                {
                    IdDocumentoVenta = objBL_DocumentoVenta.Inserta_EC(objDocumentoVenta, lstDocumentoVentaDetalle, IdEstadoCuenta2);
