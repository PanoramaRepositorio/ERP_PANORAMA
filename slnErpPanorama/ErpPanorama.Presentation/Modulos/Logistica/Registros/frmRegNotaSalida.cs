...
namespace ErpPanorama.Presentation.Modulos.Logistica.Registros
{
    public partial class frmRegNotaSalida : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void tlbMenu_EditClick()
        {
            ...
            XtraMessageBox.Show("Opción no disponible con este documento", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Stop);
            return;
        }
        ...
        private void FilaDoubleClick(GridView view, Point pt)
        {
            ...
            if (info.InRow || info.InRowCell)
            {
                if (gvMovimientoAlmacen.RowCount > 0)
                {
                    ...
                    frmRegNotaSalidaEdit objManMovimientoAlmacenEdit = new frmRegNotaSalidaEdit();
                    objManMovimientoAlmacenEdit.pOperacion = frmRegNotaSalidaEdit.Operacion.Consultar;
                    objManMovimientoAlmacenEdit.IdMovimientoAlmacen = objMovimientoAlmacen.IdMovimientoAlmacen;
                    objManMovimientoAlmacenEdit.StartPosition = FormStartPosition.CenterParent;
                    objManMovimientoAlmacenEdit.btnGrabar.Enabled = false;
                    if (objManMovimientoAlmacenEdit.ShowDialog() == DialogResult.OK)
        ...
        {
            bool Revision = bool.Parse(gvMovimientoAlmacen.GetFocusedRowCellValue("FlagRevision").ToString());
            bool FlagEstado = bool.Parse(gvMovimientoAlmacen.GetFocusedRowCellValue("FlagEstado").ToString());
            string Numero = gvMovimientoAlmacen.GetFocusedRowCellValue("Numero").ToString();
            int IdAlmacenDestino = int.Parse(gvMovimientoAlmacen.GetFocusedRowCellValue("IdAlmacenDestino").ToString());
                ...
                if (!Revision)
                {
                    if (IdAlmacenDestino == Parametros.intAlmCentralUcayali)
                    {
                        ...
                        if (frmAutoriza.Edita)
                        {
                            ...
                            objBL_MovimientoAlmacen.ActualizaRevision(IdMovimientoAlmacen, true, frmAutoriza.Usuario);
                            XtraMessageBox.Show("Nota de Salida Cambió de estado a REVISIÓN", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    {
                        XtraMessageBox.Show("No se puede REVISAR, la nota de salida debe tener otro destino\nPor favor verifique!", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    }
                }
            }
            else
        ...
        private void actualizarBultosEnNSToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (gvMovimientoAlmacen.RowCount > 0)
            {
                //MovimientoAlmacenBE objMovimientoAlmacen = new MovimientoAlmacenBE();
                ...
                MovimientoAlmacenBE objE_UpdBultos = null;
                objE_UpdBultos = new MovimientoAlmacenBL().SelFlagUpdBultos(Parametros.intEmpresaId, Convert.ToInt32(gvMovimientoAlmacen.GetFocusedRowCellValue("IdMovimientoAlmacen").ToString()));

                if (objE_UpdBultos != null)
                {
                    if (objE_UpdBultos.FlagUpd)
                    {
                        XtraMessageBox.Show("Ya se realizo la actualización de Bultos en esta nota de salida.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                }


                frmActualizarBultosPedidos objActualizaBultosPed = new frmActualizarBultosPedidos();

                objActualizaBultosPed.qIdPedido = int.Parse(gvMovimientoAlmacen.GetFocusedRowCellValue("IdMovimientoAlmacen").ToString());
                objActualizaBultosPed.qNumPedido =  gvMovimientoAlmacen.GetFocusedRowCellValue("Numero").ToString();
                objActualizaBultosPed.qBultos = int.Parse(gvMovimientoAlmacen.GetFocusedRowCellValue("Bultos").ToString());
                objActualizaBultosPed.StartPosition = FormStartPosition.CenterParent;

                if (Parametros.strUsuarioLogin == "master" || Parametros.strUsuarioLogin == "almacen2" 
                    || Parametros.intPerfilId == Parametros.intPerAsistenteAlmacen
                    || Parametros.intPerfilId == Parametros.intPerAuxiliarAlmacen
                ...
                {
                    if (objActualizaBultosPed.ShowDialog() == DialogResult.OK)
                    {
                        Cargar();
                    }
                }
                ...
                {
                    XtraMessageBox.Show("No tiene acceso a la opción.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
