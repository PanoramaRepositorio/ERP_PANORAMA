...
INSERT INTO Menu ( MenuCodigo, IdMenuPadre, MenuDescripcion, Imagen, LargeImage, Clase, Ensamblado, IdMenuTipo, ModoCargaVentana, FlagEstado)
VALUES ('00130101', 2457, 'Cotizaci�n', 'Consultas_32x32', 1, 'frmRegKiraCotizacion', 'ErpPanorama.Presentation.Modulos.KiraHogar.Registros.frmRegKiraCotizacion', 2, 1, 1);
INSERT INTO Menu ( MenuCodigo, IdMenuPadre, MenuDescripcion, Imagen, LargeImage, Clase, Ensamblado, IdMenuTipo, ModoCargaVentana, FlagEstado)
...
-- Insertar datos a TABLA ELEMENTO (TIPO COTIZACI�N)

INSERT INTO TablaElemento (IdTabla, Abreviatura, DescTablaElemento, IdTablaExterna, Valor, FlagEstado)
VALUES (105, 'CPC', 'COTIZACI�N PARA EL CLIENTE', NULL, NULL, 1);
...
INSERT INTO TablaElemento (IdTabla, Abreviatura, DescTablaElemento, IdTablaExterna, Valor, FlagEstado)
VALUES (108, 'ILMN', 'ILUMINACI�N', NULL, NULL, 1);
INSERT INTO TablaElemento (IdTabla, Abreviatura, DescTablaElemento, IdTablaExterna, Valor, FlagEstado)
...
INSERT INTO TablaElemento (IdTabla, Abreviatura, DescTablaElemento, IdTablaExterna, Valor, FlagEstado)
VALUES (110, 'CPT', 'PASAJE DISE�ADORA', NULL, NULL, 1);
INSERT INTO TablaElemento (IdTabla, Abreviatura, DescTablaElemento, IdTablaExterna, Valor, FlagEstado)
...
CREATE TABLE CotizacionKIRA (
    ...
    FOREIGN KEY (Moneda) REFERENCES TablaElemento (IdTablaElemento) -- Relaci�n con la tabla TablaElemento para la columna Moneda
);
...
(
    ...
    @Moneda INT, -- Nuevo par�metro para la moneda
    @DetalleCotizacion dbo.DetalleCotizacionType READONLY,
    @IdCotizacion INT OUTPUT,
    @FlagEstado BIT -- Nuevo par�metro para el FlagEstado de la Cotizaci�n
)
AS
BEGIN
    SET NOCOUNT ON;

    -- Insertar la cotizaci�n en la tabla "CotizacionKIRA"
    ...
    -- Obtener el ID de la cotizaci�n reci�n insertada
    SET @IdCotizacion = SCOPE_IDENTITY()

    -- Insertar los detalles de cotizaci�n desde la tabla estructurada a la tabla "DetalleCotizacion"
...
-- Declarar una tabla para almacenar los detalles de cotizaci�n
DECLARE @DetalleCotizacionData AS dbo.DetalleCotizacionType;

-- Insertar datos en la tabla de detalles de cotizaci�n (puedes ajustar estos valores seg�n tus necesidades)
...
EXEC usp_RegistrarCotizacionYDetalle
    ...
    @Caracteristicas = 'MUEBLE BAR FLOTANTE / ESTRUCTURA: MELAMINA PELIKANO ROVERE CON FONDO DE ESPEJO / PUERTAS: VIDRIO AHUMADO / ILUMINACI�N: LED DICROICO LUZ C�LIDA / MEDIDAS: 2.16m largo x 0.36m profundidad x 0.91m alto',
    ...
    @DetalleCotizacion = @DetalleCotizacionData, -- Datos de los detalles de cotizaci�n
    @IdCotizacion = @IdCotizacionResult OUTPUT, -- Variable de salida para el IdCotizacion
    @FlagEstado = 1; -- Valor para el nuevo par�metro FlagEstado (1 para activo)
...
--    -- Actualizamos el CodigoProducto y la Descripci�n utilizando el procedimiento almacenado
--    UPDATE CotizacionKIRA
...
BEGIN
    ...
    BEGIN
        PRINT 'El C�digo de Producto ' + @CodigoProducto + ' ya existe en la base de datos.';
        SET @ExisteCodigo = 1;
    END
    ELSE
    BEGIN
        PRINT 'El C�digo de Producto ' + @CodigoProducto + ' no existe en la base de datos.';
        SET @ExisteCodigo = 0;
    END
END
GO
...
---- C�digo de producto no existente:
...
----    -- Actualizar detalles de la cotizaci�n si es necesario
...
----        @IdCotizacion = 4,  -- Cambia esto al ID de la cotizaci�n que deseas actualizar
----        @NuevoCodigoProducto = 'SPORTINGX ',
----        @NuevaDescripcion = 'Nueva descripci�n',
----        @NuevoCaracteristicas = 'Nuevas caracter�sticas',
...
----        @NuevaDescripcionGastos = 'Nueva descripci�n de gastos',
----        @NuevoFlagAprobacion = 1,  -- Cambia esto al valor correcto de aprobaci�n
...
BEGIN TRY
    EXEC usp_ActualizarCotizacion
        @IdCotizacion = 1,  -- Cambia esto al ID de la cotizaci�n que deseas actualizar
...
BEGIN
    ...
    -- Aqu� escribe la l�gica para obtener los detalles de la cotizaci�n por su IdCotizacion
...
--- Obtiene detalleCotizacion PESTA�AS
...
(
    @IdCotizacion INT,
	@IdTablaElemento INT, -- Agrega este par�metro
...
END;
-- Ejemplo de ejecuci�n de usp_AgregarDetalleCotizacion
DECLARE @IdCotizacion INT = 2; -- Cambia el valor seg�n la cotizaci�n a la que deseas agregar el detalle
DECLARE @IdTablaElemento INT = 715;
DECLARE @DescripcionGastos NVARCHAR(200) = 'Descripci�n de gastos de prueba';
DECLARE @Costo DECIMAL(18, 2) = 100.00; -- Cambia el costo seg�n el valor deseado

EXEC usp_AgregarDetalleCotizacion @IdCotizacion, @IdTablaElemento, @DescripcionGastos, @Costo;
-- Consulta para verificar los detalles de cotizaci�n relacionados con una cotizaci�n
...
VALUES (8), (11), (1); -- Agrega aquí los valores de prueba que desees
